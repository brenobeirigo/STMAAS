...
class Dao(object):
        ...
        for p1 in network.keys():
            for m in network[p1].keys():
                for p2 in network[p1][m].keys():
                    distance_matrix[(p1, p2, m)] = int(network[p1][m][p2]/m_s)
    ...
    def __init__(self):
        ...
        G = load_network(filename=self.network_path[1], folder=self.network_path[0])
        #dist = create_distance_data(G)
        
        dist = load_dist_dic(self.network_path[0] +"/"+ self.network_path[1])
        ...
        print("Loading distances from '{0}'".format(self.network_path), len(dist))
    ...
    def create_earliest_latest_dic(self):
        ...
        earliest_tstamp = int(Vehicle.start_revealing_tstamp)
...
class DaoHybrid(Dao):
        ...
        with open(self.vehicles_list_path) as f:
            ...
            for row in reader:
                ...
                veh = Vehicle(av_id,
                                ...
                                acquisition_cost = config.vehicle_instances["MODE_INFO"][type_vehicle]["fixed_cost"],
    ...
    def __init__(self,
        ...
        for r in self.request_list:
            ...
            self.nodes_dic_nw[r.get_destination().get_network_id()] = r.get_destination()
        ...
        self.earliest_t.update({v.get_pos().get_id():v.get_available_at() for v in self.vehicle_dic.values()})

        # Define earliest latest times to attend request
        self.earliest_tstamp = {p.get_origin().get_id(): p.get_revealing_tstamp()
                                for p in self.request_list}
       
        self.earliest_tstamp.update({v.get_pos().get_id():v.get_available_at_tstamp() for v in self.vehicle_dic.values()})
        
        # Define max pick-up delay for each node
        self.max_pickup_delay = {p.get_origin().get_id(): p.get_pickup_delay()
                                 for p in self.request_list}

        self.max_pickup_delay.update({v.get_pos().get_id():0 for v in self.vehicle_dic.values()})
        
        # Define max pick-up delay for each node
        self.max_delivery_delay = {p.get_origin().get_id(): p.get_delivery_delay()
                                 for p in self.request_list}

        self.max_delivery_delay.update({v.get_pos():0 for v in self.vehicle_dic.values()})
