...
class ScatterPlot:
    ...
    def create_scatter_plot(self):
        nodes_dic = self.DAO.nodes_dic
        # Create list of x coordinates
        x_coord = [nodes_dic[key].coord.x * 1000000
                   for key in nodes_dic.keys()]

        # Create list of y coordinates
        y_coord = [nodes_dic[key].coord.y * 1000000
                   for key in nodes_dic.keys()]
    ...
    def create_arrows(self):
        vehicle_dic = self.DAO.vehicle_dic
        for k in vehicle_dic.keys():
            ...
            while p1 in path.keys() and p2 != None:
                # From/to coordinates
                p1_coord = path[p1].coord
                p2_coord = path[p2].coord

                # x, y coordinates From/to
                p1_x = p1_coord.x * 1000000
                p1_y = p1_coord.y * 1000000
                p2_x = p2_coord.x * 1000000
                p2_y = p2_coord.y * 1000000
    ...
    def create_middle_line_annotation(self):
        ...
        for k in vehicle_dic.keys():
            ...
            while p1 in path.keys() and p2 != None:
                ...
                x_ij = '$' + p1[0:2] + "_{" + p1[2:] + \
                    '},' + p2[0:2] + "_{" + p2[2:] + '}' + '$'
                plt.annotate(str(x_ij + '(' + str(round(self.DAO.distance_matrix[p1, p2] / 60, 2)) + ')'),
                             (middle_p.x * 1000000,
                              middle_p.y * 1000000),
                             xytext=(0, 0),
                             textcoords='offset points',
    ...
    def create_requests(self):
        requests = self.DAO.request_list
        for h in requests:
            p1 = h.origin
            p1_coord = p1.coord
            p2 = h.destination
            ...
            id_origin = p1.id
            id_destination = p2.id

            # Pickup
            plt.annotate("$" + str(h.id) + "$" + '(' + str(h.get_demand_short()) + ')',
                         (p1_coord.x * 1000000, p1_coord.y * 1000000),
                         xytext=(0, 30),
                         textcoords='offset points',
                         ha='center',
                         va='bottom',
